...
import { User, AlertTriangle, X, Trash2, ShieldAlert, Mail, Building } from "lucide-react"
import deleteUser from "../../../../functions/admin/users/deleteUser"

const DeleteUser = ({ user, users, setUsers, isOpen, onOpenChange }) => {
    ...
    const handleDelete = async () => {
        if (confirmText === 'confirm delete') {
            setIsLoading(true)
            const result = await deleteUser(user?.id, users, setUsers)
            if (result === 200)
                onOpenChange(false)
            setIsLoading(false)
        }
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    <p className="text-red-700 text-xs">{user?.email}</p>
                                                </div>
                                                {user?.center && (
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <Building className="w-4 h-4 text-red-500" />
                                                        <p className="text-red-700 text-xs">{user.center.name}</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                <ul className="text-amber-700 text-sm space-y-1">
                                                    ...
                                                    <li className="flex items-center gap-2">
                                                        <div className="w-1.5 h-1.5 bg-amber-600 rounded-full"></div>
                                                        All user data and records will be removed
                                                    </li>
                                                    <li className="flex items-center gap-2">
                                                        <div className="w-1.5 h-1.5 bg-amber-600 rounded-full"></div>
