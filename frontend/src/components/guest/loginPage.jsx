...
import { Modal, ModalContent, ModalHeader, ModalBody, useDisclosure, Button, Input, Select, SelectItem, Avatar, Badge, AutocompleteItem, Autocomplete } from "@heroui/react"
...
const AuthPage = () => {
  ...
  const toggleFormMode = () => {
    setIsLogin(!isLogin)
    setFormErrors({})
    setFormData({ email: "", password: "", firstName: "", surname: "", country: "", professional_country: "", centerId: null, gender: "", researchPosition: "", bornDate: "" })
  ...
  return (
    <Modal backdrop="blur" isOpen={isOpen} onOpenChange={handleOpenChange} placement="center" size="4xl" scrollBehavior="outside" classNames={{ base: "bg-white dark:bg-gray-900", closeButton: `hover:bg-cyan-50 text-gray-500 cursor-pointer ${isLogin ? "" : "text-white"} hover:text-cyan-600` }}>
      <ModalContent>
        <div className="flex flex-col md:flex-row rounded-lg overflow-hidden">
          ...
          <div className={`w-full md:w-1/2 p-5 ${isLogin ? "md:order-1" : ""}`}>
            <ModalHeader className="flex flex-col gap-1 text-center relative p-0 mb-3">
              {!isLogin && (
                <button
                  ...
                  className="absolute left-0 top-1/2 transform -translate-y-1/2 p-2 rounded-full hover:bg-cyan-50 transition-colors cursor-pointer text-cyan-600"
                >
                  <ArrowLeft size={20} />
                </button>
              )}
                ...
                <div className="flex justify-center mb-2">
                  <div className="p-3 bg-gradient-to-r from-cyan-100 to-emerald-100 rounded-full">
                    <div className="p-4 bg-white/20 rounded-full backdrop-blur-sm">
                      <img className="w-36 h-36 rounded-full" src="/edi/equicenter.png" alt="EquiCenter Logo" />
                    </div>
                  </div>
                </div>
              )}
            ...
            <ModalBody className="p-0">
              <AnimatePresence mode="wait Forgot password?">
                {isLogin ? (
                  ...
                  <motion.div key="register-form" variants={registerVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }} className="space-y-1">
                    ...
                    <Autocomplete
                      ...
                      onChange={(e) => handleInputChange("professional_country", e.target.value)}
                      ...
                      variant="bordered"
                    >
                      {(country) => <AutocompleteItem startContent={<Avatar alt={country.key} className="w-6 h-6" src={country.flag} />} key={country.key}>{country.label}</AutocompleteItem>}
                    </Autocomplete>
                    ...
                    <Button
                      isLoading={isLoading}
                      className="bg-gradient-to-r from-cyan-500 to-emerald-500 text-white font-bold w-full mt-2 shadow-md hover:from-cyan-600 hover:to-emerald-600"
                      onPress={handleSubmit}
                      size="lg"
                    >
                      Create Account
          ...
          <div
            className={`w-full md:w-1/2 bg-gradient-to-br from-cyan-500 to-emerald-500 p-8 flex flex-col justify-center items-center text-white ${isLogin ? "" : "md:order-2"
              }`}
          >
            <div className="text-center">
              {!isLogin && (
                <div className="mb-6 flex justify-center">
                  <div className="p-4 bg-white/20 rounded-full backdrop-blur-sm">
                    <img
                      className="w-40 h-40 border-white border-2 rounded-full"
              ...
              <h2 className="text-3xl font-bold mb-4">{isLogin ? "Welcome!" : "Welcome Back!"}</h2>
              ...
              <Button
                color="default"
                className="bg-white/10 hover:bg-white/20 rounded-full px-8 py-6 text-white font-medium border-2 border-white/30 backdrop-blur-sm transition-all"
